...
struct MonthlyView: View {
    ...
    @State private var showingDayView: Bool = false
    ...
    var body: some View {
        ...
        if showingDayView {
            ZStack(alignment: .bottomLeading) {
                DayView()
                Button("Back", systemImage: "arrowshape.turn.up.backward.circle.fill", action: {
                   showingDayView = false
                }).labelStyle(.iconOnly).font(.largeTitle).foregroundStyle(appModel.headerColor)
            ...
            VStack(spacing: appModel.moduleSpacing) {
                VStack(spacing: spacingScale) { // Use spacingScale for Vstack spacing
                    HStack {
                        ...
                        ZStack {
                            ...
                            }).font(.caption).padding(5).bold().background(appModel.headerColor).clipShape(RoundedRectangle(cornerRadius: 10)).foregroundStyle(.white).padding(.bottom, 10)
                        }
                        Spacer()
                        ...
                        if layoutType == .elsePortrait || layoutType == .iphonePortrait {
                            ForEach(viewModel.days, id: \.self) { day in
                                ...
                                ZStack(alignment: .leading) {
                                    ...
                                    Rectangle()
                                        .foregroundStyle(day.isCurrentDay ? appModel.headerColor : Color.clear).frame(maxWidth: .infinity, maxHeight: .infinity)
                ...
                HStack {
                    VStack {
                        SpaceView(type: "Tasks", scale: .medium, layoutType: .elsePortrait)
                        
                       
                    }
                    VStack{
                        
                        SpaceView(type: "Notes", scale: .medium, layoutType: .elsePortrait)
